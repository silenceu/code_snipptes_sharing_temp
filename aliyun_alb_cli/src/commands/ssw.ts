...
export default class SSW extends Command {
    ...
    static examples = ["$ aliyun_alb_cli ssw -g SERVER_GROUP_ID"];
    ...
    async run() {
        ...
        const ConfigFileBuffer = await fs.readFileSync(
            path.join(this.config.configDir, "config.json")
        );
        const Config = JSON.parse(ConfigFileBuffer.toString());
        const { flags } = this.parse(SSW);
        const serverGroupId: string =
            flags.serverGroupId || Config.serverGroupId;
        try {
            const albClient = new AlbClient(
                Config.accessEndpoint,
                Config.accessKeyId,
            ...
            const serverGroupServers = await albClient.getServerGroupServers(
                serverGroupId
            );
            ...
            forEach(serverGroupServers, (serverGroupServer) => {
                this.log(
                    `服务${colors.green(
                        serverGroupServer.description || "--"
                    )}(${serverGroupServer.serverIp}:${
                        serverGroupServer.port
                    }), 权重值[${colors.green(
                        String(serverGroupServer.weight)
                    )}]`
                );
            });
            ...
            await albClient.setServerWeight(
                serverGroupId,
                updateServer,
                weight
            );
            ...
            await new Promise((resolve) => {
                const timer = setInterval(async () => {
                    ...
                    if (percentage === 100) {
                        clearInterval(timer);
                        const serverGroupServers =
                            await albClient.getServerGroupServers(
                                serverGroupId
                            );
                        const updatedServer = find(
                            serverGroupServers,
                            (serverGroupServer) => {
                                return (
                                    serverGroupServer.serverIp ===
                                    ...
                                    serverGroupServer.weight === weight
                                );
                            }
                        );
                        if (updatedServer) {
                            ...
                            this.log(
                                ...
                                )}(${updatedServer.serverIp}:${
                                    updatedServer.port
                                })设置权重为${colors.green(weight)}成功`
                            );
                            resolve({});
                        }
                    }
